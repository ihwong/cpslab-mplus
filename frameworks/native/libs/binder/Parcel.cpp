...
String16 Parcel::getInterface(IPCThreadState* threadState) const
{
...
void Parcel::writeRemoteUid(int32_t remoteUid)
{
	size_t orgDataPos = mDataPos;
	mDataPos = 0;
	readInt32();
	String16 interface = readString16();
	if(interface != String16("android.os.IServiceManager")){
		int magicNum = readInt32();
		if (magicNum == MAGIC_NUMBER)
			writeInt32(remoteUid);
	}
	mDataPos = orgDataPos;
}
