...
class ActivityStarter {
	...
	/* mobileplus start */
	int remoteExecute(int remoteCallerHandle, ComponentName remoteCallerName) {
		try {
			return remoteStartActivityMayWait(mRequest.caller, mRequest.callingUid,
                        mRequest.callingPackage, mRequest.realCallingPid, mRequest.realCallingUid,
                        mRequest.intent, mRequest.resolvedType,
                        mRequest.voiceSession, mRequest.voiceInteractor, mRequest.resultTo,
                        mRequest.resultWho, mRequest.requestCode, mRequest.startFlags,
...
/* mobileplus start */
    private int remoteStartActivityMayWait(IApplicationThread caller, int callingUid,
            String callingPackage, int requestRealCallingPid, int requestRealCallingUid,
            Intent intent, String resolvedType, IVoiceInteractionSession voiceSession,
            IVoiceInteractor voiceInteractor, IBinder resultTo, String resultWho, int requestCode,
            int startFlags, ProfilerInfo profilerInfo, WaitResult outResult,
            ...
            boolean allowPendingRemoteAnimationRegistryLookup,
            PendingIntentRecord originatingPendingIntent, boolean allowBackgroundActivityStart, 
			int remoteCallerHandle, ComponentName remoteCallerName) {
        ...
        synchronized (mService.mGlobalLock) {
            ...
            final ActivityRecord[] outRecord = new ActivityRecord[1];
            int res = remoteStartActivity(caller, intent, ephemeralIntent, resolvedType, aInfo, rInfo,
                    voiceSession, voiceInteractor, resultTo, resultWho, requestCode, callingPid,
                    callingUid, callingPackage, realCallingPid, realCallingUid, startFlags, options,
	...
	private int remoteStartActivity(IApplicationThread caller, Intent intent, Intent ephemeralIntent,
            String resolvedType, ActivityInfo aInfo, ResolveInfo rInfo,
            IVoiceInteractionSession voiceSession, IVoiceInteractor voiceInteractor,
            IBinder resultTo, String resultWho, int requestCode, int callingPid, int callingUid,
            String callingPackage, int realCallingPid, int realCallingUid, int startFlags,
		...
		callerApp = new  WindowProcessController(mService, null, "com.example.virtual_callerApp", -1, 0, null, null); 
        ...
        ActivityRecord sourceRecord = null;
        ActivityRecord resultRecord = null;
		sourceRecord= mRootActivityContainer.getTopDisplayFocusedStack().getTopActivity();
		if (sourceRecord != null){
			sourceRecord.remoteCallerName = remoteCallerName;
			sourceRecord.remoteResultTo = resultTo;
			sourceRecord.remoteCallerHandle = remoteCallerHandle;
			if(requestCode >= 0 && !sourceRecord.finishing){
				resultRecord = sourceRecord;
			}

			Slog.v("MOBILEPLUS(ActivityStarter)", "remoteStartActivity() remoteCallerName="+remoteCallerName+", remoteResultTo="+resultTo+", remoteCallerHandle="+remoteCallerHandle+", to sourceRecord="+sourceRecord+", aInfo.exported="+aInfo.exported);
		}
