...
public class ActivityTaskManagerService extends IActivityTaskManager.Stub {
    ...
    int startActivityAsUser(IApplicationThread caller, String callingPackage,
		...
		Slog.v("MOBILEPLUS(ActivityTaskManagerService)", "startActivityAsUser(), caller:"+caller+", callingPackage:"+callingPackage+", intent:"+intent+", resolvedType:"+resolvedType+", resultTo:"+resultTo+", resultWho:"+resultWho+", requestCode:"+requestCode+", startFlags:"+startFlags+", profilerInfo:"+profilerInfo+", bOptions:"+bOptions+", userId:"+userId+", validateIncomingUser:"+validateIncomingUser);
        // TODO: Switch to user app stacks here.
    ...
    public final boolean finishActivity(IBinder token, int resultCode, Intent resultData,
            int finishTask) {
		Slog.v("MOBILEPLUS(ActivityTaskManagerSevice)", "finishActivity(), token="+token+", resultCode="+resultCode+", resultData="+resultData);
        // Refuse possible leaked file descriptors
    ...
    public ComponentName getCallingActivity(IBinder token) {
        synchronized (mGlobalLock) {
            ActivityRecord r = getCallingRecordLocked(token);
			/* mobileplus start */
			if (r != null && r.remoteCallerName != null)
				return r.remoteCallerName;
			/* mobileplus end */
	...
	/* mobileplus start */
	public int startVirtualActivity(IApplicationThread caller, IBinder resultTo, int requestCode)
	{
		Intent intent = new Intent();
		ComponentName cn = new ComponentName("com.mobileplus.systemui", "com.mobileplus.systemui.VirtualActivity");
		intent.setComponent(cn);
		String resolvedType = null;
		String callingPackage = cn.getPackageName();
		String resultWho = null;
		int startFlags = 0;
		ProfilerInfo profilerInfo=null;
		Bundle bOptions = null;

		int ret = startActivity(caller, callingPackage, intent, resolvedType, resultTo, resultWho, requestCode, startFlags, profilerInfo , bOptions);
		return ret;
	}

	@Override
	public final int remoteStartActivity(IApplicationThread caller, String callingPackage,
			Intent intent, String resolvedType, IBinder resultTo, String resultWho, int requestCode,
			int startFlags, ProfilerInfo profilerInfo, Bundle bOptions, int remoteCallerHandle, ComponentName remoteCallerName){
		...
		startActivity(null /*caller*/, "com.mobileplus.systemui" /*callingPackage*/, newIntent, 
						resolvedType, null /*resultTo*/, resultWho, requestCode, 
						startFlags, profilerInfo, bOptions); 
			
		Slog.v("MOBILEPLUS(ActivityTaskManagerService)", "remoteStartActivity() caller="+caller+", callingPackage="+callingPackage+", resultTo="+resultTo+", requestCode="+ requestCode+", remoteCallerHandle="+remoteCallerHandle+", remoteCallerName="+remoteCallerName);
		return remoteStartActivityAsUser(caller, callingPackage, intent, resolvedType, resultTo,
				resultWho, requestCode, startFlags, profilerInfo, bOptions, 
				UserHandle.getCallingUserId(), true, 
			    remoteCallerHandle, remoteCallerName);
		
	}

	int remoteStartActivityAsUser(IApplicationThread caller, String callingPackage,
			Intent intent, String resolvedType, IBinder resultTo, String resultWho, int requestCode,
			int startFlags, ProfilerInfo profilerInfo, Bundle bOptions, 
			int userId, boolean validateIncomingUser, int remoteCallerHandle, ComponentName remoteCallerName)
	{
		enforceNotIsolatedCaller("startActivityAsUser");
		userId = getActivityStartController().checkTargetUser(userId, validateIncomingUser, 
						Binder.getCallingPid(), Binder.getCallingUid(), "startActivityAsUser");
		...
		return getActivityStartController().obtainStarter(intent, "startActivityAsUser")
                ...
                .setActivityOptions(bOptions)
                .setMayWait(userId)
                .remoteExecute(remoteCallerHandle, remoteCallerName);
	}

 
	public  boolean finishVirtualActivity(IBinder token, List<ResultInfo> results)
	{
		ActivityRecord caller = mRootActivityContainer.isInAnyStack(token);
		TaskRecord targetTask = caller.getTaskRecord();
		ActivityRecord r = targetTask.getTopActivity();
		ResultInfo resultData = results.get(0);
		boolean ret= finishActivity(r.appToken, resultData.mResultCode, resultData.mData, 0);
		return ret;
	}
	/* mobileplus end */
}
