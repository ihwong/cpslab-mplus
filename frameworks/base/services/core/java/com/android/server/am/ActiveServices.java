...
public final class ActiveServices {
    ...
    int bindServiceLocked(IApplicationThread caller, IBinder token, Intent service,
		...
		/* mobileplus start */
		//Slog.v("MOBILEPLUS(ActiveService.java)", "bindServiceLocked with callerApp.appzygote "+callerApp.appZygote + ", uid " + callerApp.uid+", removed "+callerApp.removed +", userId "+callerApp.userId);
		/* mobileplus end */
		...
		Slog.v("MOBILEPLUS(ActiveServices.java)", "callerApp : "+callerApp +" res:"+ res);
	...
	int remoteBindIsolatedServiceLocked(IApplicationThread caller, IBinder token, Intent service,
					String resolvedType, final IServiceConnection connection, int flags,
					String instanceName, String callingPackage, final int userId)
					throws TransactionTooLargeException {

	
	Slog.v("MOBILEPLUS(ActiveServices.java)", "remoteBindIsolatedServiceLocked");
	if (DEBUG_SERVICE) Slog.v(TAG_SERVICE, "remoteBindService: " + service 
					+ " type=" + resolvedType + " conn=" + connection.asBinder() 
					+ " flags=0x" + Integer.toHexString(flags));
	ProcessRecord callerApp = new ProcessRecord(mAm, "mobileplus");	

	final boolean callerFg = true;
	final boolean isBindExternal = (flags & Context.BIND_EXTERNAL_SERVICE) != 0;
	final boolean allowInstant = (flags & Context.BIND_ALLOW_INSTANT) != 0;
	...
	try {
		if(unscheduleServiceRestartLocked(s, -1, false)){
		...
		if ((flags&Context.BIND_AUTO_CREATE) != 0){
			s.lastActivity = SystemClock.uptimeMillis();
			if(bringUpServiceLocked(s, service.getFlags(), callerFg, false, false) != null){
				return 0;
			}
		}
		...
		if (s.app != null && b.intent.received) {
			...
			// publish the connection.
			try {
				Slog.v("MOBILEPLUS(ActiveServices", "remoteBindServiceLocked(), s.name="+s.name +
						", b.intent.biner="+b.intent.binder+", service="+service);
				c.conn.connected(s.name, b.intent.binder, false);	
			} catch (Exception e){
				Slog.w(TAG, "Failure sending service "+s.shortInstanceName
							+ " to connection "+ c.conn.asBinder()
