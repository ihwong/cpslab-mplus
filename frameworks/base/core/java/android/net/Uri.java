...
public abstract class Uri implements Parcelable, Comparable<Uri> {
    ...
    */
	...
	/* mobileplus start */
	/** @hide */
	public static void writeToParcel(Parcel out, Uri uri, int uriNumPos){
		if (uri == null){
			out.writeInt(NULL_TYPE_ID);
		} else {
			int origPos = out.dataPosition();

			out.setDataPosition(uriNumPos);
			int uriNum = out.readInt();
			for (int i=0; i<uriNum; i++){
				int writePos = out.dataPosition();
				if(out.readInt()==0){
					out.setDataPosition(writePos);
					Slog.v("MOBILEPLUS(Uri.java)", "write Index at "+writePos+", value: "+origPos);
					out.writeInt(origPos);
					break;
				}
			}

			out.setDataPosition(origPos);
			uri.writeToParcel(out, 0);
		}
