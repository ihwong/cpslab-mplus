...
interface IActivityTaskManager {
	...
	/* mobileplus start */
	int startVirtualActivity(in IApplicationThread caller, in IBinder resultTo, in int requestCode);
	boolean finishVirtualActivity(in IBinder caller, in List<ResultInfo> results);	
	int remoteStartActivity(in IApplicationThread caller, in String callingPackage, 
					in Intent intent, in String resolvedType, in IBinder resultTo, in String resultWho, in int requestCode,
					in int startFlags, in ProfilerInfo profilerInfo, in Bundle options, in int remoteCallerHandle, in ComponentName remoteCallerName );
