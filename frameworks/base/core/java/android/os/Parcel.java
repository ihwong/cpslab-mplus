...
public final class Parcel {
	...
	/** @hide */
	public final void writeBundle(@Nullable Bundle val, int uriNumPos){
		if (val == null){
			writeInt(-1);
			return ;
		}

		val.writeToParcel(this, 0, uriNumPos);
	}
	/** @hide */
	/* package */ void writeArrayMapInternal(@Nullable ArrayMap<String, Object> val, int uriNumPos){
		if (val == null){
			writeInt(-1);
			return;
		}
		...
		for (int i=0; i<N; i++){
			if (DEBUG_ARRAY_MAP) startPos = dataPosition();
			writeString(val.keyAt(i));
			writeValue(val.valueAt(i), uriNumPos);
	...
	/** @hide */
	public final void writeValue(@Nullable Object v, int uriNumPos){
        if (v == null) {
            writeInt(VAL_NULL);
        } else if (v instanceof String) {
            writeInt(VAL_STRING);
            ...
            writeInt(VAL_PARCELABLE);
			if (v instanceof Uri)
				writeParcelable((Parcelable) v, 0, uriNumPos);
			else
            	writeParcelable((Parcelable) v, 0);
        } else if (v instanceof Short) {
            writeInt(VAL_SHORT);
	...
	/** @hide */
	public final void writeParcelable(@Nullable Parcelable p, int parcelableFlags, int uriNumPos)
	{
        if (p == null) {
            writeString(null);
            return;
		...
		//write the position of uri
		int origPos = dataPosition();
		setDataPosition(uriNumPos);
		int uriNum = readInt();
