...
public class BaseBundle {
   ...
   public int getInt(String key, int defaultValue) {
	...
	/* mobileplus start */
	void writeToParcelInner(Parcel parcel, int flags, int uriNumPos) {
        // If the parcel has a read-write helper, we can't just copy the blob, so unparcel it first.
        if (parcel.hasReadWriteHelper()) {
            unparcel();
        }
		...
		//Store the position of the bundle length
		parcel.setDataPosition(uriNumPos - 4);		// mobileplus
		Slog.v("MOBILEPLUS(BaseBundle.java)", "write Idx at "+parcel.dataPosition()+", value: "+lengthPos);
		parcel.writeInt(lengthPos);					// mobileplus
		parcel.setDataPosition(lengthPos);			// mobileplus
        ...
        int startPos = parcel.dataPosition();
			//parcel.writeArrayMapInternal(map);
		parcel.writeArrayMapInternal(mMap, uriNumPos);		// mobileplus
        int endPos = parcel.dataPosition();
